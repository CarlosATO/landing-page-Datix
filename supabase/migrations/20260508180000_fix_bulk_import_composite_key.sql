-- Fix bulk import composite key to include forma_farmaceutica.
-- Prevents false duplicates when same nombre+dci have different
-- concentration or pharmaceutical form.
...
begin
  ...
  loop
    ...
    begin
      ...
      v_composite_key := v_name || '|' || v_dci || '|' || coalesce(v_concentration, '') || '|' || coalesce(v_presentation, '');
      if v_composite_key = any(v_seen_composites) then
        raise exception 'nombre+dci+concentracion+forma_farmaceutica duplicado en archivo';
      end if;
