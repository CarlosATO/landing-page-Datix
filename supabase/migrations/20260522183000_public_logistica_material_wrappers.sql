create or replace function public.list_catalog_categories(p_payload jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  v_category_type := nullif(btrim(coalesce(p_payload->>'category_type', '')), '');
  ...
  select coalesce(jsonb_agg(to_jsonb(c) order by c.name asc), '[]'::jsonb)
    into v_categories
  from (
    select
      cc.id,
    ...
    where cc.company_id = v_company_id
      and (
        v_search is null
        or upper(cc.code) like '%' || upper(v_search) || '%'
        or upper(cc.name) like '%' || upper(v_search) || '%'
      )
      and (v_category_type is null or lower(cc.category_type) = lower(v_category_type))
      and (
        not v_is_active
        or coalesce(cc.is_active, true) = true
      )
...
create or replace function public.deactivate_logistica_material(p_item_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public, logistica
...
begin
  ...
  select * into v_item from logistica.items li where li.id = p_item_id;
  ...
  return jsonb_build_object('success', true, 'logistica_item_id', p_item_id);
end;
$$;

grant execute on function public.list_catalog_categories(jsonb) to authenticated;
...
grant execute on function public.deactivate_logistica_material(uuid) to authenticated;
