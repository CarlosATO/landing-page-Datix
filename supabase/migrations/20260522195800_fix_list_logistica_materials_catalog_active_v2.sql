...
begin
  ...
  from (
    select
      ...
      li.name,
      li.description,
      coalesce(ci.item_kind, case li.item_type when 'consumable' then 'physical' else li.item_type end) as item_kind,
      li.unit,
      li.tracks_lot,
      li.tracks_serial,
    ...
    where li.company_id = v_company_id
      ...
      and (
        ...
        or upper(coalesce(li.description, '')) like '%' || upper(v_search) || '%'
      )
      and (v_item_kind is null or lower(coalesce(ci.item_kind, case li.item_type when 'consumable' then 'physical' else li.item_type end)) = v_item_kind)
