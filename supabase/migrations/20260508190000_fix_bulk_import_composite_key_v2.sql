-- Fix bulk import composite key to include laboratory name.
-- Further prevents false duplicates when same nombre+dci have different
-- concentration, presentation or laboratory.
  ...
  loop
    ...
    begin
      ...
      if v_composite_key = any(v_seen_composites) then
        raise exception 'producto duplicado en archivo (nombre+dci+concentracion+forma+laboratorio)';
      end if;
