-- Internal DTE Architecture (Simulation Mode)
-- Preparing for future SII integration without external dependencies.

-- 1. Tables
CREATE TABLE IF NOT EXISTS "pharmacy"."dte_documents" (
    "id" uuid DEFAULT gen_random_uuid() PRIMARY KEY,
    "company_id" uuid NOT NULL,
    "sale_id" uuid NOT NULL,
    "customer_id" uuid,
    "dte_type" text NOT NULL, -- BOLETA, FACTURA, NOTA_CREDITO, NOTA_DEBITO
    "folio" bigint NOT NULL,
    "status" text DEFAULT 'PENDING' NOT NULL, -- PENDING, GENERATED, SENT, ACCEPTED, REJECTED, CANCELLED
    ...
    CONSTRAINT "dte_documents_status_check" CHECK ("status" = ANY (ARRAY['PENDING', 'GENERATED', 'SENT', 'ACCEPTED', 'REJECTED', 'CANCELLED'])),
    CONSTRAINT "dte_documents_type_check" CHECK ("dte_type" = ANY (ARRAY['BOLETA', 'FACTURA', 'NOTA_CREDITO', 'NOTA_DEBITO']))
);

CREATE TABLE IF NOT EXISTS "pharmacy"."dte_folios" (
    "company_id" uuid NOT NULL,
    "dte_type" text NOT NULL,
    "current_folio" bigint DEFAULT 0 NOT NULL,
    "is_active" boolean DEFAULT true NOT NULL,
    PRIMARY KEY (company_id, dte_type)
);
...
CREATE POLICY "DTE Documents Isolation" ON "pharmacy"."dte_documents"
    FOR ALL TO authenticated USING (company_id = (SELECT company_id FROM public.company_users WHERE user_id = auth.uid() LIMIT 1));

CREATE POLICY "DTE Folios Isolation" ON "pharmacy"."dte_folios"
    FOR ALL TO authenticated USING (company_id = (SELECT company_id FROM public.company_users WHERE user_id = auth.uid() LIMIT 1));

-- 3. RPC to generate internal DTE
CREATE OR REPLACE FUNCTION "pharmacy"."generate_internal_dte"("p_sale_id" "uuid") RETURNS "uuid"
...
begin
    ...
    -- Por ahora solo BOLETA para ventas POS
    v_dte_type := 'BOLETA';

    -- 2. Obtener y actualizar folio (lock para evitar duplicados)
...
-- 4. Update process_pharmacy_sale to include DTE generation
...
begin
  ...
  loop
    ...
    loop
      ...
      where id = v_batch.id
        and company_id = v_company_id
        and current_quantity >= v_qty_to_deduct;
  ...
  -- GENERAR DTE INTERNO AUTOMÁTICAMENTE
